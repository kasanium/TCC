...
def lk():
    # matriz de rigidez do elemento (BENDSOE2004 p.268 ou ANDREASSEN2011 p. 15 - ambos de XX line optimization code, ANDREASSEN adaptou p fins de vetorização)
    E, nu = 1.0, 0.3

    k = np.array([1/2-nu/6, 1/8+nu/8, -1/4-nu/12, -1/8+3*nu/8,
                  -1/4+nu/12, -1/8-nu/8, nu/6, 1/8-3*nu/8]) # componentes do array, linha 90 do BENDSOE 
    ...
    return E/(1-nu**2)*np.array([
        ...
        [k[7], k[2], k[1], k[4], k[3], k[6], k[5], k[0]]]) # montagem da matriz de rigidez em estado plano de tensões

def precompute_filter(nelx, nely, rmin):
    # pre-calcula a matriz do filtro H - filtros de independência da malha e fatores de normalização
    # FIXME: se a malha for mt grande, essa matriz densa vai estourar a memoria.
    # pensar em usar scipy.sparse DEPOIS (se deu certo deixa p depois kkkkk)

    if rmin <= 0:
        print("aviso: rmin zerado ou negativo, filtro inativo na pratica")

    H = np.zeros((nelx * nely, nelx * nely)) # linha 29 do código do ANDREASSEN2011 (p.6 too), 49 do BENDSOE2004
    for i1 in range(nelx):
        for j1 in range(nely):
            e1 = i1 * nely + j1

            for i2 in range(max(i1 - int(rmin), 0), min(i1 + int(rmin) + 1, nelx)): # otimização da busca da vizinhança do filtro (bounding box) - p. 263 <--- EXPLICACOES AQUI
                for j2 in range(max(j1 - int(rmin), 0), min(j1 + int(rmin) + 1, nely)):
                    e2 = i2 * nely + j2
                    dist = np.sqrt((i1 - i2)**2 + (j1 - j2)**2)
                    H[e1, e2] = max(0, rmin - dist) 

    return H, np.sum(H, 1) # fatores de normalização

def FE(nelx, nely, x, penal):
    KE = lk()
    e0 = 1.0 
    emin = 1e-9 # linha 5 do ANDREASSEN2011, prevenir matriz de rigidez se tronando singular
    ...
    for elx in range(nelx):
        for ely in range(nely):
            ...
            # dofs (degrees of freedom - 2) do elemento (repetitivo mesmo)
    ...
    # carga pontual no finalzinho (cantilever)
    F[ndof - 1, 0] = -1.0 
...
def sensitivity_analysis(nelx, nely, x, penal, U):
    ...
    e0, emin = 1.0, 1e-9 # rep de antes
    
    for elx in range(nelx):
        for ely in range(nely):
            n1 = (nely+1)*elx + ely
            n2 = (nely+1)*(elx+1) + ely
            edof = np.array([2*n1, 2*n1+1, 2*n2, 2*n2+1, 2*n2+2, 2*n2+3, 2*n1+2, 2*n1+3]) # mapeamento do graus de liberdade para elemento q4 nós; "1 and 2n correspond to the horizontal and vertical displacement of node n, respectively"
            
            Ue = U[edof, 0]
            ce = Ue.T @ KE @ Ue # energia mutua / compliance do elemento
            idx = ely + elx*nely
            
            c += (emin + (x[idx]**penal) * (e0 - emin)) * ce # contribuicao do elemento à compilancia global usando a interpolação simp
...
def optimality_criteria_update(nelx, nely, x, volfrac, dc, H, Hs):
    # filtro das sensibilidades - p.3 do ANDREASSEN2011
    # max(1e-3, x) pra evitar divisao por 0
    dc = (H @ (x * dc)) / Hs / np.maximum(1e-3, x)
    
    l1, l2 = 0.0, 100000.0
    move = 0.2 # SIGMUND2001 usa esse default
    ...
    while (l2 - l1) / (l1 + l2) > 1e-3:
        ...
        # ps: arrumei pra usar sqrt(-dc/lmid) q nem no script do matlab SIGMUND2001 p.4, linha do código 42
        xnew = np.maximum(0.001, 
               np.maximum(x - move, 
               np.minimum(1.0, 
               np.minimum(x + move, x * np.sqrt(-dc / lmid)))))
        
        if xnew.mean() > volfrac: # linha 43 SIGMUND2001
            l1 = lmid
        else: 
            l2 = lmid
...
if __name__ == "__main__":
    ...
    nelx, nely = 32, 20 # tava precisando de resultado p comparar, chamei KHARMANDO2001 pro deck - valores tudo na p. 302, example 2; v. tabela de resultados na p. 303
    volfrac = 0.4
    penal = 3.0
    rmin = 1.2 # e é para dar compliance 57.3492
    ...
    # limitando a 100 pq demora mt no meu cpu se deixar rolar
